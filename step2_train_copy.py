...
for fold, (tr_idx, va_idx) in enumerate(cv_iter, start=1):
    ...
    for key, label, params in model_specs:
        ...
        if key == "xgb":
            ...
            # XGBoost supports sample_weight for class imbalance if needed
            ...
            # LightGBM with class_weight='balanced' inside params; still OK to pass weights
            sw = np.array([cw[c] for c in ytr])
            model.fit(Xtr, ytr, sample_weight=sw)
            proba = model.predict_proba(Xva)
        elif key == "cat":
            model = CatBoostClassifier(**params)
            # CatBoost: class_weights as list aligned by class id
