...
def _human_rationale(row, feat_names, shap_vals, margins, pred_set, veto_hit,
                     pconf_raw=None, pconf_veto=None):
    parts = []
    if veto_hit:
    ...
    # Feature rationale (top drivers)
    if isinstance(shap_vals, np.ndarray):
        L = min(len(shap_vals), len(feat_names))
        idx = np.argsort(-np.abs(shap_vals))[:min(5, L)]
        ranked = []
        for j in idx:
            if j < L:  # guard against out-of-bounds
                ranked.append(f"{feat_names[j]} ({'+' if shap_vals[j]>=0 else '−'}{abs(shap_vals[j]):.2f})")
        if ranked:
...
# ---------- Main API ----------
def run_step5(max_rows=None, out_prefix="step5", top_k=None):
    """
    ...
    with open(expl_path, "w") as f:
        for i in range(len(X)):
            ...
            if shap_vec is not None:
                L = min(len(shap_vec), len(feature_cols))
                idx = np.argsort(-np.abs(shap_vec))[:min(TOP_FEATURES_PER_SAMPLE, L)]
                top_feats = []
                for j in idx:
                    if j < L:  # guard against out-of-bounds
                        top_feats.append({
    ...
    # Derived probability for visualization
    df_out["p_exoplanet"] = df_out["p_conf"]  # treat Confirmed probability as p_exoplanet

    # Make sure p_exoplanet exists, is finite, and in [0,1]
    if "p_exoplanet" not in df_out.columns:
        df_out["p_exoplanet"] = df_out["p_conf"]
    df_out["p_exoplanet"] = pd.to_numeric(df_out["p_exoplanet"], errors="coerce").clip(0,1)
    ...
    # Attach RA/Dec/Dist + key physics from unified parquet for 3D Universe
    df_coords = pd.read_parquet(UNIFIED_PARQUET)[
        ["star_id", "ra_deg", "dec_deg", "dist_pc", "rade_Re", "period"]
    ]
    df_out = df_out.merge(df_coords, on="star_id", how="left") 
    if "period" in df_coords.columns:
        df_out["period"] = df_coords["period"]    

    # --- Mission text from flags or unified parquet (if you later add it there) ---
    if "mission" not in df_out.columns:
        if all(c in df_out.columns for c in ["is_kepler","is_k2","is_tess"]):
            df_out["mission"] = np.select(
                [df_out["is_kepler"] == 1, df_out["is_k2"] == 1, df_out["is_tess"] == 1],
                ["Kepler", "K2", "TESS"],
                default="Unknown"
            ).astype(str)
        else:
            df_out["mission"] = "Unknown"  

    # Filter rows that have valid coordinates for 3D placement
    coord_mask = df_out[["ra_deg","dec_deg","dist_pc"]].notnull().all(axis=1)
    placed = coord_mask.sum()
    total  = len(df_out)
    print(f"[Step5] 3D-placeable rows: {placed}/{total}")

    df_out_3d = df_out.loc[coord_mask].copy()  
    # Cartesian positions from RA/Dec/Distance (ICRS; distance in parsecs)
    ra_rad  = np.deg2rad(df_out_3d["ra_deg"].to_numpy())
    dec_rad = np.deg2rad(df_out_3d["dec_deg"].to_numpy())
    R       = pd.to_numeric(df_out_3d["dist_pc"], errors="coerce").to_numpy()

    df_out_3d["x"] = R * np.cos(dec_rad) * np.cos(ra_rad)
    df_out_3d["y"] = R * np.cos(dec_rad) * np.sin(ra_rad)
    df_out_3d["z"] = R * np.sin(dec_rad)
 
    # Optional: limit to top-K for demo
    if top_k:
        df_out = df_out.head(top_k)
        print(f"[Step5] Exporting top {top_k} candidates for demo.")
    ...
    # 🚀 Fix: ensure mission flags exist to avoid KeyError
    for flag in ["is_kepler", "is_k2", "is_tess"]:
    ...
    # 🔭 Export full universe rows (with positions + probs) for UI
    with open("reports/step5_universe.json", "w") as f:
        json.dump(df_out_3d.to_dict(orient="records"), f, indent=2)
    print(f"[Step5] Universe JSON rows: {len(df_out_3d)}")
    ...
    summary.update({
        ...
        "discovery_formula": "score = 1.0*p_conf + 0.4*p_cand - 0.6*p_fp + 0.15*depth_snr + 0.10*geom_transit_prob - 0.10*(1 - 1/(1+|dur_consistency|))"
    })
    (summary_path).write_text(json.dumps(summary, indent=2))
    # Minimal universe payload for the frontend (with positions + probabilities)
    universe_cols = [
        "star_id","mission","pred_text","p_exoplanet","p_conf","p_cand","p_fp",
        "ra_deg","dec_deg","dist_pc","rade_Re","period","x","y","z"
    ]
    path_universe = REPORTS / "step5_universe.json"
    df_out_3d[universe_cols].to_json(path_universe, orient="records", indent=2)
    print(f"🌌 Universe payload → {path_universe}")
    ...
    # Reuse global SHAP plots from Step 3 if available
    try:
        step3_reports = ROOT / "reports"
        for fname in ["step3_shap_summary_confirmed.png", "step3_shap_bar_confirmed.png"]:
            fsrc = step3_reports / fname
            fdst = REPORTS / f"{out_prefix}_{fname}"
            if fsrc.exists():
                import shutil
                shutil.copy2(fsrc, fdst)
    except Exception as e:
        print(f"[WARN] Could not copy Step 3 SHAP plots: {e}")
    return {
...
if __name__ == "__main__":
    ...
    ap.add_argument("--top_k", type=int, default=None,
                    help="Export only top-K candidates for demo")
    args = ap.parse_args()
    ...
    run_step5(max_rows=args.max_rows, out_prefix=args.out_prefix, top_k=args.top_k)        
